...
当前封面有调研网页、数据采集、类型划分、宏观行为分析、微观行为分析五个入口。宏观目录为八个白底图表，数据采集内含目录PDF及两张源表。运行 `python scripts/build_site.py` 更新全站，同时重建根目录的 `data_extract.csv`；统计公式见 [METHODS.md](METHODS.md)，当前可编辑Python文件在 `chart_python/current`。
...
07–11 栏的人群图表来自工作簿的“行人信息总表（0907）”；10 栏的空间指标按点位 ID 关联“全量总表（0907）”。居民指数、游客指数取点位内行人原始指数的均值；身份倾向按两项指数高低分类，不代表真实居住身份。点位置信区间的最低样本量为5，相关矩阵不做FDR校正。
...
2026-09-08：统一读取0907两张总表及点位均值。类型划分含原始SVG交互总览；宏观8个板块包含界面品质、活力度、混合度信息表及11个案例。点位年龄构成仅展示n≥4。运行 scripts/verify_september.cjs 检查新导航及案例图。
