/* All study inputs come from the 0906 worksheet. Missing observations stay missing. */
const D=window.SURVEY, WHITE='#f5f5f2', ALPHA=.5, LINE_ALPHA=.2;
...
document.querySelector('h1').textContent=String(page).padStart(2,'0')+' '+titles[page-1];
document.title=titles[page-1]+'｜巨富长街区';
document.querySelector('.meta').textContent='0906 / '+D.rows.length+' RECORDS';
let charts=[];
...
function base(){return {animation:false,backgroundColor:'#090909',textStyle:{color:WHITE,fontFamily:'Arial, Microsoft YaHei'},tooltip:{confine:true,backgroundColor:'#181818',borderColor:'#777',textStyle:{color:WHITE}},grid:{left:85,right:35,top:70,bottom:100},xAxis:axis(),yAxis:axis()};}
...
function heat(i){
  ...
  const c=chart(),o=base(),columns=12,rowCount=Math.ceil(D.nodes.length/columns);
  o.grid={left:20,right:20,top:75,bottom:20};
  ...
  const cells=D.nodes.flatMap((n,j)=>n[key]==null?[]:[[j%columns,Math.floor(j/columns),n[key],n.address,n.short,n.street]]);
  o.series=[
    ...
    {name:D.headers[key],type:'heatmap',data:cells,
      ...
      label:{show:true,color:'#090909',fontSize:10,lineHeight:13,formatter:p=>p.data[2]+'人'}}
  ];
  ...
  note.textContent=`拖动上方双端滑块筛选人数范围（0–${max} 人）。节点按街道、门牌号顺序连续排列；筛选仅显隐格子，位置不变。暗色底格保留地址位置，缺失观测不按 0 人处理。`;
}
...
if(page===6)select(['遮荫率','声环境舒适度','气味环境'],i=>{scatterNote();scatter('T',['AE','AF','AG'][i],['遮荫率','声环境舒适度','气味环境'][i]);});
if(page===7)single();
