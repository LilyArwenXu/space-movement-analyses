...
function mapView(mode){note(mode===0?'选择活动查看原点位人数；圈越大人数越多，悬停时同一道路同时变实。\n圆半径 r=3+22×√(该点人数/当前指标最大人数)；按底图宽高的百分比定位。\n活动人数读取总表对应人数列；缺失观测仅在此地图按0显示。':'蓝色表示人数少，玫红色表示人数多；色块叠加展示人群在底图上的集中区域。\n权重 w=该点行人样本数/全域最大行人样本数。\n热度 h(x,y)=Σ[w_i×exp(−距离²/(2×σ²))]，σ=底图宽度的1.8%；热度按全图最大值归一化，缺失人数仅在此地图按0处理。');
 ...
 function draw(){svg.replaceChildren();const key=mode?'AH':selected,max=Math.max(1,...D.points.map(p=>p.values[key]||0));
  if(mode){const canvas=document.createElement('canvas');canvas.width=700;canvas.height=495;const ctx=canvas.getContext('2d'),values=new Float32Array(700*495);let peak=0;const sigma=12.6;
   ...
   const pixels=ctx.createImageData(700,495),colors=[[96,126,149],[168,195,214],[243,238,232],[213,155,168],[164,86,104]];
 ...
 if(!mode){['AU','AV','AX','BA','AY','BB'].forEach(k=>{const b=el('button','',tools);b.textContent=D.headers[k];b.setAttribute('role','tab');b.setAttribute('aria-selected',String(k===selected));b.onclick=()=>{selected=k;tools.querySelectorAll('button').forEach(v=>v.setAttribute('aria-selected',String(v===b)));draw();};});}draw();if(mode){const legend=el('div','heat-legend');legend.innerHTML='低人数 <span class="heat-gradient"></span> 高人数';}
}

function weights(){note('每行代表一条道路，六段长度表示各项评分均值；悬停查看评分及所占比例。\n道路某项均值=该道路有效评分之和/有效点位数。\n遮荫评分=遮荫率(%)/20。\n六项合计=开放度+互动性+视觉丰富度+历史感知度+遮荫评分+路面状态，满分30。\n某项构成比例=该项均值/六项均值合计×100%。');const c=chart(W,Math.max(650,D.weights.length*29+120)),o=base(),names=['界面开放度','临街互动性','视觉丰富度','历史感知度','遮荫率','路面状态'];o.grid={left:100,right:30,top:70,bottom:35};o.xAxis=axis();o.yAxis={type:'category',data:D.weights.map(g=>g.road),axisLabel:{fontFamily:FONT,interval:0},inverse:true};o.series=names.map((name,j)=>({name,type:'bar',stack:'total',data:D.weights.map(g=>g.values[j]),itemStyle:{color:['#292929','#4B4B4B','#A8C3D6','#B8AEA6','#A45668','#A8C3D6'][j],opacity:j===5?.55:1},label:{show:true,color:j<2?'#fff':'#292929',formatter:p=>fmt(p.value,1)}}));o.tooltip.trigger='axis';o.tooltip.formatter=ps=>esc(ps[0].name)+'<br>'+ps.map(p=>`${esc(p.seriesName)}：${fmt(p.value,2)} (${fmt(p.value/sum(ps.map(v=>v.value))*100,1)}%)`).join('<br>');c.setOption(o);}
...
function lorenz(parent,g){const c=chart(parent,480),o=base(),order=g.actions.map((v,j)=>({v,j})).sort((a,b)=>a.v-b.v||a.j-b.j);o.title={text:'行为集中度：洛伦兹曲线',left:'center',top:5,textStyle:{fontFamily:FONT,fontSize:16}};o.legend={...o.legend,top:40,left:10,right:10};o.grid={left:55,right:20,top:92,bottom:115,containLabel:true};o.xAxis={...axis(),type:'category',data:['起点',...order.map(p=>D.behaviorLabels[p.j])],axisLabel:{rotate:55,interval:0,fontSize:13,fontFamily:FONT}};o.yAxis={...axis('行为出现次数累计比例'),min:0,max:1};o.series=[{name:'均等线',type:'line',data:Array.from({length:15},(_,i)=>i/14),symbol:'none',lineStyle:{color:'#A8C3D6',type:'dashed'}},{name:g.name,type:'line',data:g.lorenz||[],symbolSize:4,lineStyle:{color:'#292929'},itemStyle:{color:'#292929'}}];o.tooltip.trigger='axis';c.setOption(o);}
...
function correlationColor(value){if(!finite(value))return '#F3EEE8';if(value>0){const a=value<.2?[243,238,232]:[213,155,168],b=value<.2?[213,155,168]:[164,86,104],t=value<.2?value/.2:Math.min(1,(value-.2)/.3);return 'rgb('+a.map((v,i)=>Math.round(v+(b[i]-v)*t)).join(',')+')';}const stops=['#607E95','#A8C3D6','#B8AEA6','#F3EEE8','#E2D0BC','#D59BA8','#A45668'],n=(value+1)*3,i=Math.min(5,Math.floor(n)),f=n-i,hex=h=>[1,3,5].map(k=>parseInt(h.slice(k,k+2),16)),a=hex(stops[i]),b=hex(stops[i+1]);return `rgb(${a.map((v,k)=>Math.round(v+(b[k]-v)*f)).join(',')})`}
