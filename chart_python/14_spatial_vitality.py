...
function correlationMatrix(){
  const el=document.createElement('section');el.className='report reference-matrix combined-correlations';workspace.append(el);
  ...
  el.innerHTML='<h2>Spearman图表</h2>'+el.innerHTML+'<h2 class="coefficient-heading">Spearman相关系数</h2>'+referenceTable(rows);
}
...
function report(i){
 ...
 if(i===1){correlationMatrix();return;}
 const el=document.createElement('section');el.className='report';workspace.append(el);
 if(i===2){el.innerHTML=referenceTable(REFERENCE[2]);return;}
...
if(page===3)bars();
if(page===4)select(['数据概况','Spearman相关系数','显著相关对','分析结论'],report);
