...
ROUTE = 'scatter_single_axis.html'
SOURCE_SCRIPT = 'studies.js'
HTML = r'''<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>不配得性Ⅰ：界面评价/行为选择｜衡复风貌区</title><link rel="stylesheet" href="studies.css"><script src="echarts.min.js"></script><script src="survey-data.js"></script></head>
<body data-study="7" data-number="5" data-title="不配得性Ⅰ：界面评价/行为选择"><header><a href="../visualizations.html" aria-label="返回图表目录">←</a><h1>05 不配得性Ⅰ：界面评价/行为选择</h1><span class="meta"></span></header><nav role="tablist" aria-label="切换分析图表"></nav><p class="note"></p><main id="workspace" aria-label="数据图表"></main><script src="studies.js"></script></body></html>'''
...
function single(){const c=chart(80+D.streets.length*110),o=base();delete o.xAxis;delete o.yAxis;delete o.grid;o.singleAxis=[];o.series=[];o.graphic=[];const max=Math.max(...D.streets.map(s=>D.rows.filter(r=>r.street===s).length));D.streets.forEach((s,i)=>{const rows=D.rows.filter(r=>r.street===s),top=70+i*110;o.singleAxis.push({type:'value',min:-max/2,max:max/2,left:'20%',right:'13%',top,height:0,axisLabel:{show:false},axisTick:{show:false},splitLine:{show:false},axisLine:{lineStyle:{color:'#444'}}});o.graphic.push({type:'text',left:'7%',top:top-6,style:{text:s,fill:'#ddd',font:'12px SimSun'}});['AH','T'].forEach(key=>o.series.push({name:s,type:'scatter',coordinateSystem:'singleAxis',singleAxisIndex:i,data:rows.flatMap((r,j)=>Number.isFinite(r[key])?[[j-(rows.length-1)/2,r[key],r.address,key]]:[]),symbolSize:v=>key==='AH'?Math.max(Math.sqrt(v[1])*12,8):Math.max(v[1]*4,2),itemStyle:{color:key==='AH'?WHITE:'transparent',opacity:key==='AH'?.4:1,borderColor:'#f5f5f2',borderType:'solid',borderWidth:key==='AH'?0:1.5},emphasis:{disabled:true}}));});o.tooltip.formatter=p=>`${esc(p.data[2])}<br>${esc(D.headers[p.data[3]])}：${p.data[1]}`;c.setOption(o);note.textContent='实心圆大小表示行人样本数，空心圆大小表示界面整体状态评分；各街道颜色与透明度一致。上下滚动查看各街道，缺失指标不绘制。';}
