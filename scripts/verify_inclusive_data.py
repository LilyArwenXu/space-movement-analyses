...
_,raw=load_sheet('全量总表','全量总表0906')
_,praw=load_sheet('行人信息总表','行人信息总表0905')
original=raw
from source_adapter import adapt
raw,praw=adapt(raw,praw)
byid={r['B']:r for r in raw[3:] if r.get('B')}
new_byseq={r['A']:r for r in original[3:] if isinstance(r.get('A'),(int,float))}
for p in points:
    assert p['values']['AH']==byid[p['id']].get('AH')
    source=new_byseq[p['values']['A']]
    if original[2].get('B')=='完整地址':
        assert p['address']==source['B'] and p['values']['AH']==source.get('AI')
        assert p['values']['facility']==source.get('K')
        assert p['values']['BH']==source.get('BI')
    ...
    if p['vitality'] is not None:
        ...
        assert abs(p['vitalityRaw']-sum(parts)/4)<1e-12
        assert abs(p['vitality']-min(1,data['vitalityCoefficient']*sum(parts)/4))<1e-12
...
assert len(data['fields'])==24 and len(data['ys'])==11 and len(data['correlations'])==264
assert data['qualityFields']==['R','U']
assert data['dualHigh']==sum(p['quadrant']=='双高' for p in points)>=8
for point in points:
    parts=[]
    for key in data['qualityFields']:
        values=[p['values'][key] for p in points if finite(p['values'].get(key))]
        if finite(point['values'].get(key)):
            parts.append((point['values'][key]-min(values))/(max(values)-min(values)))
    if len(parts)==len(data['qualityFields']):
        assert abs(point['quality']-sum(parts)/len(parts))<1e-12
    else:assert point['quality'] is None
